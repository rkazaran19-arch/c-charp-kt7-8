# 📘 Практика по C#: Делегаты, Action/Func, Лямбды

Набор практических заданий для отработки тем: делегаты, групповой вызов, ковариантность/контрвариантность (вводно), `Action<T>` и `Func<T>`, лямбда-выражения.
